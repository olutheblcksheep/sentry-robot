...
Obstacle detection is paused during path replay so the recorder
...
def check_obstacles(scan):
    ...
    Skips during path replay and when auto mode is off."""
    if not state.auto_mode:
        return

    # Don't interrupt a timed path replay
    try:
        import routes
        if routes._replay_running:
            return
    except Exception:
        pass
...
def lidar_loop():
    ...
    if not config.LIDAR_ENABLED or config.MOCK_MODE:
        print("[LiDAR] Disabled or mock mode — skipping")
        return
    try:
        ...
        while True:
            ...
            while len(buf) >= 47:
                ...
                if scan:
                    ...
                    try:
                        import pathfinder
                        pathfinder.update_grid_from_scan(scan)
                    except Exception:
                        pass
